...
public class TelemetryConfigService implements TelemetryConfigProvider {
    ...
    private Map<String, Object> getPaywallConfigs(Stack stack) {
        Map<String, Object> paywallConfigs = new HashMap<>();
        String accountId = Crn.safeFromString(stack.getResourceCrn()).getAccountId();
        UserManagementProto.Account account = umsClient.getAccountDetails(accountId, MDCUtils.getRequestId());
        Optional<String> licenseOpt = Optional.of(account.getClouderaManagerLicenseKey());
        licenseOpt.flatMap(cmLicenseParser::parseLicense).ifPresent(jsonLicense -> {
            ...
            if (isNotEmpty(username) && isNotEmpty(password)) {
                paywallConfigs.put("paywall_username", username);
                paywallConfigs.put("paywall_password", password);
            }
        });
