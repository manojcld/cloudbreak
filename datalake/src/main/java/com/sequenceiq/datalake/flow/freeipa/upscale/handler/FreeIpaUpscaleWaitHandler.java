...
public class FreeIpaUpscaleWaitHandler extends ExceptionCatcherEventHandler<FreeIpaUpscaleWaitRequest> {
    ...
    protected Selectable doAccept(HandlerEvent<FreeIpaUpscaleWaitRequest> event) {
        ...
        try {
            ...
            response = new FreeIpaUpscaleFailedEvent(sdxId, userId,
                    new PollerStoppedException("FreeIpa Upgrade timed out after " + durationInMinutes + "minues"));
    ...
    public void waitFreeIpaUpscale(String envCrn, String operationId, PollingConfig pollingConfig) {
        ...
        Polling.waitPeriodly(pollingConfig.getSleepTime(), pollingConfig.getSleepTimeUnit())
                ...
                .run(() -> {
                    LOGGER.info("Deletion polling freeIpa for operation status: '{}' in '{}' env", operationId, envCrn);
                    Optional<OperationStatus> freeIpaOperation = ThreadBasedUserCrnProvider.doAsInternalActor(() ->
                            freeipaService.getFreeIpaOperation(operationId, accountId));
                    if (freeIpaOperation.isEmpty()) {
                        LOGGER.info("Stack status of operation {} by response from freeIpa: NOT FOUND", operationId);
                        return AttemptResults.justContinue();
                    }
                    OperationStatus operationStatus = freeIpaOperation.get();
                    LOGGER.info("Stack status of operation {} by response from freeIpa: {}", operationId,
                    ...
                    if (OperationState.FAILED == operationStatus.getStatus()) {
                        ...
                        return AttemptResults.breakFor("FreeIpa Upscale failed");
                    }
                    return AttemptResults.justContinue();
                });
    }
